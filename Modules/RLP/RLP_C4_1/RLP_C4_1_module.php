...
class RLP_C4_1_module extends Module
{
    /**
     * @var array
     */
    ...
    public function __construct(School $school,  SchoolClass $school_class, Student $student, Federal $federal, string $module_name, SchoolTemplateVariables $school_template_variables)
    {
        ...
        $this->competences = $this->model->getCompetences();
        ...
        if ($this->getIsAjax())
        {
            ...
            foreach ($this->competences as $key => $competence)
            {
                if (!empty($_POST[$key]))
                {
                    $this->competences[$key] = $_POST[$key];
                }
            }
            ...
            $this->model->setCompetences($this->competences);
